...
-- 4.
alter table people add constraint unique(email);

-- 5
alter table pets change column name first_name varchar(255);
...
select first_name, phone_number from customer 
where gender='F' and last_name like '%Bluth';



-- 8.
select name from products where price>3.00 or coffee_origin="INDIA";

-- 9.
select Count(*) from customer where gender='M' and phone_number is NULL;

-- 10.
select name,price from products where coffee_origin in ('COLOMBIA','INDNESIA','INDONESIA') order by name;

-- 11.
select * from orders where order_time like "%2017-02-%" and id in (2,4,6,8);
